...
def test_calculate_time_and_days():
    ...
    # Match the expected result with the logic of descending order by average_per_day
    expected = [
...
def test_missing_out_event():
    ...
    expected_time_and_days = [
        {"user_id": "123", "time": 0.0, "days": 0, "average_per_day": 0.0, "rank": 1}
    ]
    result = calculate_time_and_days(data)
    print("Result:", result)
    assert result == expected_time_and_days

def test_consecutive_in_out_events():
    data = [
        {"user_id": "123", "event_type": "GATE_IN", "event_time": datetime(2023, 1, 1, 8, 0)},
        {"user_id": "123", "event_type": "GATE_IN", "event_time": datetime(2023, 1, 1, 9, 0)},  # Consecutive IN
        {"user_id": "123", "event_type": "GATE_OUT", "event_time": datetime(2023, 1, 1, 12, 0)},
    ]
    expected_time_days = [
    ...
    result = calculate_time_and_days(data)
    print("Result:", result)
    assert result == expected_time_days
